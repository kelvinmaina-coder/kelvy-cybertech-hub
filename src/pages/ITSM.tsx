...
interface TicketData {
  ...
  status: string; client_id: number | null; assigned_to: string | null;
  created_by: string | null; created_at: string; updated_at: string;
}

const priorityColors: Record<string, string> = {
  critical: "text-destructive bg-destructive/10", high: "text-warning bg-warning/10",
  medium: "text-secondary bg-secondary/10", low: "text-muted-foreground bg-muted/30",
};
const statusColors: Record<string, string> = {
  open: "text-warning bg-warning/10", in_progress: "text-secondary bg-secondary/10",
  resolved: "text-primary bg-primary/10", closed: "text-muted-foreground bg-muted/30",
};
...
export default function ITSM() {
  ...
  const loadData = async () => {
    ...
    setTickets((ticketRes.data as TicketData[]) || []);
  ...
  const addTicket = async (e: React.FormEvent) => {
    ...
    setForm({ title: "", description: "", priority: "medium", client_id: "", assigned_to: "" });
    setShowAdd(false);
  };
  ...
  const assignTicket = async (id: number, userId: string) => {
    await supabase.from("tickets").update({ assigned_to: userId } as any).eq("id", id);
    ...
    <div className="space-y-6">
      ...
      {showAdd && (
        <form onSubmit={addTicket} className="rounded-lg border border-warning/30 bg-card p-4 space-y-3">
          ...
          <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Description..."
            className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm font-mono min-h-[80px]" />
      ...
      <div className="flex gap-2 flex-wrap">
        <select value={filter.status} onChange={e => setFilter(p => ({ ...p, status: e.target.value }))}
          className="bg-card border border-border rounded-lg px-3 py-1.5 text-xs font-mono">
        ...
        <select value={filter.priority} onChange={e => setFilter(p => ({ ...p, priority: e.target.value }))}
          className="bg-card border border-border rounded-lg px-3 py-1.5 text-xs font-mono">
          <option value="">All Priority</option><option value="low">Low</option>
          <option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option>
        </select>
      </div>

      <div className="rounded-lg border border-border bg-card p-4">
        <h3 className="font-display text-sm text-warning mb-3">TICKETS ({filteredTickets.length})</h3>
        {loading ? (
          <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-warning" /></div>
        ) : filteredTickets.length === 0 ? (
          <p className="text-sm text-muted-foreground font-mono text-center py-8">No tickets match filters</p>
        ) : (
          <div className="space-y-1">
            {filteredTickets.map(t => (
              <div key={t.id} className="flex items-center gap-3 p-2 rounded bg-muted/20 text-sm">
                ...
                {(hasRole("super_admin") || hasRole("manager")) && (
                  <select value={t.assigned_to || ""} onChange={e => assignTicket(t.id, e.target.value)}
                    className="text-[10px] px-1 py-0.5 rounded font-mono bg-transparent border border-border max-w-[80px]">
                    <option value="">Assign</option>
                    {users.map(u => <option key={u.id} value={u.id}>{(u.full_name || "").slice(0, 10)}</option>)}
                  </select>
                )}
              </div>
            ))}
